...
const PropertiesSchema = Type.Object({
  ...
  source: Type.Optional(Type.String({ description: "Data source range, e.g. 'Sheet1!A1:D100'" })),
  range: Type.Optional(Type.String({ description: "Output location (pivot table top-left cell)" })),
  anchor: Type.Optional(Type.String({ description: "Chart placement (top-left cell)" })),
...
export const modifyObjectTool = defineTool({
  ...
  execute: async (_toolCallId, params) => {
    try {
      ...
      const message = error instanceof Error ? error.message : "Unknown error modifying object";
